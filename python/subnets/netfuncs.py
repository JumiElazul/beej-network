...
def my_tests():
    ...
    class Tester:
        ...
        def __call__(self, function, *args, expected=None, **kwargs) -> bool:
            def fmt_arg(x):
                return x if isinstance(x, str) else repr(x)

            def join_args(args, kwargs):
                parts = [fmt_arg(a) for a in args]
                if kwargs:
                    parts += [f"{k}={fmt_arg(v)}" for k, v in kwargs.items()]
                return ", ".join(parts)

            result = function(*args, **kwargs)
            passed = (result == expected)

            sig = f"{function.__name__}({join_args(args, kwargs)})"
            result_s = str(result)
            expected_s = str(expected)

            self.w_sig = max(self.w_sig, len(sig))
            self.w_res = max(self.w_res, len(result_s))
            self.w_exp = max(self.w_exp, len(expected_s))

            print(
                f"{sig:<{self.w_sig}} -> "
                f"{result_s:<{self.w_res}} "
                f"(expected {expected_s:<{self.w_exp}}) "
    ...
    test(ipv4_to_value, "255.255.0.0",     expected=4294901760)
    test(ipv4_to_value, "1.2.3.4",         expected=16909060)
    test(ipv4_to_value, "255.255.255.255", expected=4294967295)

    test(value_to_ipv4, 4294901760, expected="255.255.0.0")
    test(value_to_ipv4, 16909060,   expected="1.2.3.4")
    test(value_to_ipv4, 4294967295, expected="255.255.255.255")

    test(get_subnet_mask_value, "/16",            expected=4294901760)
    test(get_subnet_mask_value, "10.20.30.40/23", expected=4294966784)

    test(ips_same_subnet, "10.23.121.17", "10.23.121.225", "/23", expected=True)
    test(ips_same_subnet, "10.23.230.22", "10.24.121.225", "/16", expected=False)
